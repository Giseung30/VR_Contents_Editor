...
*/
    ...
    public void Update()
    {
        ...
        _hits = Physics.RaycastAll(_rayCastCenter, GetComponent<Transform>().forward, _maxDistance); //앞 쪽에서 부딪힌 물체들의 정보를 담음
        ...
        _hits = Physics.RaycastAll(_rayCastCenter, -GetComponent<Transform>().forward, _maxDistance); //뒷 쪽에서 부딪힌 물체들의 정보를 담음
        for (int i = 0; !_hitTransform && i < _hits.Length; i += 1) //부딪힌 물체들을 탐색
        {
            if (_hits[i].transform.tag != "Door") //문이 아니면
            {

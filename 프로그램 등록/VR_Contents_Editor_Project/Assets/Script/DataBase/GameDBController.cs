...
public class GameDBController : DBController
{
    ...
    public override void TotalStart(string sql)
    {
        try
        {
            ...
            using (IDbCommand cmd = connection.CreateCommand())
            {
                 ...
                 * keyDic ==> 시나리오번호
                 */
                _saveContorl = GameObject.Find("SaveContent").GetComponent<SaveController>();
                _saveContorl.playedDic = playedDic;
                _saveContorl.keyDic = keyDic;
                _saveContorl.timeDic = timeDic;
                _saveContorl.fileName = tmp;
            }
    ...
    public override void TotalDelete(string sql)
    {
        try
        {

...
public class LegAnimationButton : MonoBehaviour
{
    ...
    public void OnClickAction()
    {
        ...
        {
            _aniBarParent = _aniBarParent.transform.Find("Man" + (_clickedItemControl._clickedItem._objectNumber)).transform.GetChild(4).gameObject;
            _smallAniBarParent = _smallAniBarParent.transform.Find("Man" + (_clickedItemControl._clickedItem._objectNumber)).transform.GetChild(3).gameObject;
        }
        else if (_clickedItemControl._clickedItem._originNumber == 2000)
        {
            _aniBarParent = _aniBarParent.transform.Find("Daughter" + (_clickedItemControl._clickedItem._objectNumber)).transform.GetChild(4).gameObject;
            _smallAniBarParent = _smallAniBarParent.transform.Find("Daughter" + (_clickedItemControl._clickedItem._objectNumber)).transform.GetChild(3).gameObject;
        }
        else
        {
            _aniBarParent = _aniBarParent.transform.Find("Woman" + (_clickedItemControl._clickedItem._objectNumber)).transform.GetChild(4).gameObject;
            _smallAniBarParent = _smallAniBarParent.transform.Find("Woman" + (_clickedItemControl._clickedItem._objectNumber)).transform.GetChild(3).gameObject;
        }
        ...
        _animationBar.transform.localPosition = new Vector3(0, 0, 0); //새로운 부모에서의 로컬 위치 지정
        ...
        SmallAnimationInfoSave(_smallAnimationBar.transform.GetComponent<SmallAniBar>(), false); //_animationInfo에 정보를 저장하는 함수

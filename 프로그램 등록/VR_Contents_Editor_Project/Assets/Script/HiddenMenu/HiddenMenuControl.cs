...
*/
    ...
    [Header("Variable")]
    public string _dataBaseFilepath = "Assets/DataBase";
    ...
    {
        string folderPath = Static.STATIC.dir_path + "/Database/";
        ...
        {
            VRDBController.ConIn("Assets/DataBase/TotalTable");
        }
        /*
...
*/
    ...
    IEnumerator LoadHandItemFromDataBase()
    {
        ...
        foreach (DataRow dr in _dataBaseItem.Rows)
        {
            ...
            if (_isLeft == 1)
            {
                //if (_item._originNumber == 2002) //여자 객체인 경우 본 계층이 다르기 때문에 별도로 지정
                //{
                //    _hand = _item.item3d.transform.GetChild(0).GetChild(1).GetChild(0).GetChild(2).GetChild(0).GetChild(0).GetChild(0).GetChild(0).GetChild(0).GetChild(0).GetChild(0);
                //}
                //else
                //{
            ...
            {
                //if (_item._originNumber == 2002) //여자 객체인 경우
                //{
                //    _hand = _item.item3d.transform.GetChild(0).GetChild(1).GetChild(0).GetChild(2).GetChild(0).GetChild(0).GetChild(0).GetChild(2).GetChild(0).GetChild(0).GetChild(0);
                //}
                //else
                //{
            ...
            if (_isLeft == 1)
            {
                if (_item._originNumber == 2002) //여자 객체인 경우 남자, 딸 객체와 본이 다르기 때문에 별도로 지정해주어야한다.
                {
                    _clone.transform.position = _hand.transform.GetChild(2).GetChild(0).transform.position; //세번째 손가락의 위치가 다름
                }
                else
                {
                    _clone.transform.position = _hand.transform.GetChild(2).transform.position; //세번째 손가락 위치로 지정
                }
                _humanItem._leftHandItem = _handItem;
                _clone.transform.GetChild(0).transform.localPosition += new Vector3(0, 2, 0) * _clone.GetComponentInChildren<Collider>().bounds.extents.x; //_handObjectClone 의 위치 미세조정
                _clone.transform.rotation = _hand.rotation;
    ...
    IEnumerator LoadDressFromDataBase()
    {
        ...
        foreach (DataRow dr in _dataBaseDress.Rows)
        {
            ...
            {
                Vector3 _pos = _item.item3d.transform.localPosition;
                ...
                if (_sname[1].Equals("normal"))
                {
                    ...
                    _shoes = _item.item3d.transform.Find("shoes").transform.GetChild(0).transform.GetChild(0).gameObject;
                    _pos.y = 0f;
                }
                else if (_sname[1].Equals("abnormal"))
                {
                    _item.item3d.transform.Find("shoes").transform.GetChild(0).gameObject.SetActive(false);
                    _item.item3d.transform.Find("shoes").transform.GetChild(1).gameObject.SetActive(true);
                    _shoes = _item.item3d.transform.Find("shoes").transform.GetChild(1).transform.GetChild(0).gameObject;
                    _pos.y = 0.14f;
                }
                _item.item3d.transform.localPosition = _pos;
    ...
    IEnumerator LoadAnimationFromDataBase()
    {
        ...
        for (int i = 0; i < _dataBaseItem1.Rows.Count; i++)
        {
            ...
            Debug.Log(_animationName + " " + _barWidth);
    ...
    IEnumerator LoadVoiceFromDataBase()
    {
        ...
        for (int i = 0; i < _dataBaseItem1.Rows.Count; i++)
        {
            ...
            int _number1 = (int)Convert.ChangeType(dr1["Number"], typeof(int));
            int _humanNumber1 = (int)Convert.ChangeType(dr1["HumanNumber"], typeof(int));
            int _voiceGender = (int)Convert.ChangeType(dr1["VoiceGender"], typeof(int));
...
*/
    ...
    public void OnclickSaveSave()
    {
        ...
        if (_saveText.text != null)
        {
            ...
            {
                FileInfo fInfo = new FileInfo(_filePath + ".sqlite");
    ...
    public IEnumerator Before_CreateDB()
    {
        ...
        VRDBController.ConIn("Assets/DataBase/TotalTable");
        VRDBController.Total_add(_saveText.text, _date);
        contentNum = VRDBController.Total_GetKey(_saveText.text);
        ...
        if (newBtn)
        {
            ...
            tmp.name = _saveText.text;
            tmp.transform.GetChild(0).GetComponent<Text>().text = "<color=#0000ff>" + contentNum + ". </color>" + _saveText.text + " : <color=#ff0000>0</color>";
        ...
        else
        {
            ...
            Static.STATIC._saveClickButton.transform.GetChild(0).GetComponent<Text>().text = "<color=#0000ff>" + contentNum + ". </color>" + _saveText.text + " : <color=#ff0000>0</color>";
    ...
    public IEnumerator CreateDB()
    {
        ...
        VRDBController.Init("Assets/DataBase/" + _saveText.text);
        ...
        for (int i = 0; i < _itemListControl._dataBaseSmallAnimation.Count; i++)
        {
            try
            {
                ...
                VRDBController.addBigActionInfo(i, _barX, _barWidth, _animationName, _actionHumanNumber, _animationText);
        ...
        for (int i = 0; i < _itemListControl._voiceDBIndex; i++)
        {
            try
            {
                ...
                int _actionHumanNumber = _tmp1._item._loadHumanNumber;
                int _voiceGender = _tmp1._voiceGender;
                int _originNumber = _tmp1._item._originNumber;


                VRDBController.AddBigVoiceInfo(i, _barX, _barWidth, _voiceName, _actionHumanNumber, _voiceText);
                VRDBController.AddSmallVoiceInfo(i, _actionHumanNumber, _voiceGender, _voiceName, _barX, _barWidth, _originNumber);

...
public class DressSlot : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler,IPointerClickHandler
{
    ...
    private void Start()
    {
        _originPos = _clickItemControl._clickedItem.item3d.transform.localPosition;
    ...
    public void dressChange(GameObject _new)
    {
        ...
        if (_name[0].Equals("shirt"))
        {
            ...
            if (_isTook) //착용한 경우에만 history에 삽입
            {
                ...
                HistoryController.pushDressChangeHist(_name, _prev, this.name, _hist._objectNumber, _hist._originNumber);
            }
        }
        else if (_name[0].Equals("pant"))
        {
        ...
        else //shoes
        {
            ...
            else
            {
                ...
                _pos = _originPos + new Vector3(0, 0.05f, 0);
            }
            _clickItemControl._clickedItem.item3d.transform.localPosition = _pos;
            if (_isTook)
            {
